...
# defining functions what user want
...
# define function which takes budget and savings as inputs and connects all the other functions
def start():
    ...
    # check saving is greater than budget, if yes re-enter
    if (saving > budget):
    ...
    # Defining a function which calculates the budget based on user's needs and wants
def calculation(diff, sav):
    ...
    for i in sorted_need_d:
        if ((diff - sorted_need_d[i][1]) >= 0):
            ...
            print("\n Amount left after your ", sorted_need_d[i][0], " need is: ", diff)
        else:
            print("\n Sorry, Your ", sorted_need_d[i][0], " need can't be satisfied as it exceeds your Budget")
    ...
    for j in sorted_want_d:
        if ((diff - sorted_want_d[j][1]) >= 0):
            ...
            print("\n Amount left after your ", sorted_want_d[j][0], " want is: ", diff)

        else:
            print("\n Sorry, Your ", sorted_want_d[j][0], " want can't be satisfied as it exceeds your Budget")
    ...
    # for asking the user to re-run if he didn't like the results
    ...
    # sending the mail
    email.sendmail("xyz.com", mailr, message)
